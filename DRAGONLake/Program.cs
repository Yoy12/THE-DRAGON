...
namespace DRAGONLake
{
    public static partial class Functions
    {
        static void Main(string[] args)
        {
            ...
            Space(6);
            if (!new FileInfo("SaveData.dat").Exists)
                Save(true);
            if (!new FileInfo("SaveData2.dat").Exists)
                CertainSerialSave();
            if ((new FileInfo("SaveData.dat").Length == 0 && new FileInfo("SaveData2.dat").Length != 0) || new FileInfo("SaveData2.dat").Length == 0)
            {
                CertainSerialSave();
                CertainSerial();
            }
            ...
            while (true)
            {
                ...
                while (true)
                {
                            ...
                            if (ending[0] == false)
                            {
                                switch (Choose)
                                {
                                    ...
                                    switch (area)
                                    {
                                        ...
                                        Writer(name + ": No.", 3);
                                        Enter();
                                        Writer("CabinMaster: Look, man. I'm running on low here. I barely have enough to eat! I'll give you half off if you buy from me. Heck, I'll throw in an extra for each purchase. Please man, help a guy out here.", 3);
                                        Enter();
                                        Writer(name + ": No-", 3);
                                        Thread.Sleep(400);
                                        Space();
                                        ...
                                        Color("red");
                                        Writer(" If I were you, I would buy my bag full! The enemies outside the town are strong and fierce! And there isn't a shop for kilometers!", 4);
                                        Color("null");
                                        Space();
                                        //Logic
                                        Space();
                                        ...
                                        //rest
                                        Space();
                                        Writer("After you encountered the cheapscape, you continue on your path through sword-happy town, destroying foe after foe.");
                                        ...
                                        Space();
                                        Writer(name + ": Hey! That salesman didn't give me my extra items!", 3);
                                        Enter();
                                        MonsterEncounter((int)HEARTS, (int)SPE, (int)ATK / 2, "Lumpy", (int)EXPNeed / 3, (int)GOLD / 5, true, useClass: true, monster: n010);
                                        Space();
                                        Writer("You go back, but he's nowhere to be found.\n\nA slim, short man walks up slowly with a pretentous smirk on his face.");
                                        Enter();
                                        Writer("Slim Swordsman: Today’s weather is at its optimal setting. It’d be a waste to have to go to work on a day like this.", 3);
                                        Space();
                                        Writer(name + ": Hey, do you know where that CabinMaster went?", 3);
                                        Enter();
                                        Writer("Slim Swordsman: Levels are just numbers in this world. Strength is just an illusion.", 3);
                                        Space();
                                        Writer(name + ": What? Hell you mean by that?", 3);
                                        Enter();
                                        Writer("Slim Swordsman: There's just so much beauty in the world, you know!?", 3);
                                        Space();
                                        Writer(name + ": Huh? What?", 3);
                                        Enter();
                                        Writer("Slim Swordsman: I’d rather trust and regret, than doubt and regret.", 3);
                                        Space();
                                        Writer(name + ": Are...you okay?", 3);
                                        Enter();
                                        Writer("Slim Swordsman: Well, how would you like me to mock you, then? I take requests.", 3);
                                        Space();
                                        Writer(name + ": You know, I'll beat your arse.", 3);
                                        Thread.Sleep(600);
                                        Space();
                                        Writer("Slim Swordsman: Well! I don’t plan to lose easily.", 3);
                                        Enter();
                                        Writer(name + ": I don't care what you think.", 3);
                                        Enter();
                                        MonsterEncounter((int)HEARTS / 2, (int)SPE / 2, (int)ATK / 2, "Keyrito", (int)EXPNeed / 3, (int)GOLD / 3, true, useClass: true, monster: n011);
                                        Space();
                                        Writer(name + ": Dumbarse.", 3);
                                        Space();
                                        Writer("After the encounters with the awfully familiar swordsmen, you are almost out of ghost town, when you see another swordsman, learning over his sword, examining his pile of kills. His aura radiates with childlike, \"innocent\" happiness.");
                                        Enter();
                                        Writer(SetColor("bold") + "CLANG!!" + SetColor());
                                        Thread.Sleep(900);
                                        Space();
                                        ...
                                        Space();
                                        Writer("The mysterious rowdy swordsman raises his hands and chants an incantation as green and purple light surrounds him.\nYou where healed to Max HP!");
                                        HP = HEARTS;//yes, I know this eliminates healed hearts. No, I don't care
                                        Enter();
                                        Color("bold");
                                        Writer("Swordsman: Now, let's get this party started!" + SetColor("red") + " It's time for a DEATH DUEL!!!", 3);
                                        Enter();
                                        ...
                                        Enter();
                                        Writer("Swordsman: No matter where I looked; other swordsman, collegues from my academy, my sensei, or even monsters couldn't handle my blade. But you...you showed be something that Master Domínguez always told me...", 3);
                                        Enter();
                                        Writer("Swordsman: That everyone has a superior in their craft. And it's important that we listen to and respect them!", 3);
                                        Enter();
                                        ThrDot();
                                        Writer("Swordsman: I'm not going to do that for you though!", 4);
                                        Space();
                                        ThrDot();
                                        Writer(name + ": bruh.", 3);
                                        Enter();
                                        Writer("Mark: Anyway, my name's Mark! Te veré de nuevo!", 3);
                                        Enter();
                                        Writer(name + ": ...what?", 3);
                                        Enter();
                                        area = 3;
                                        ...
                                        Enter();
                                        Writer("Not wishing to fight in a decrepid state, you pack and embark on your journey again...");
                                        Enter();
                                        ...
                                        while (stop == false)
                                        {
                                            switch (option)
                                            {
                                                ...
                                                Enter();
                                                Writer("Mark: At least " + SetColor("bold") + " try!", 3);
                                                Color("null");
                    ...
                    else if (Chose == "2" && hasStarted)//selects new game, wiping save
                    {
                        Space();
                        Console.Clear();
                        stop = false;
